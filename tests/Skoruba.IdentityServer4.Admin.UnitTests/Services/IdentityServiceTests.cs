...
namespace Skoruba.IdentityServer4.Admin.UnitTests.Services
{
    public class IdentityServiceTests
    {
        ...
        private IIdentityService<UserDto<string>, string, RoleDto<string>, string, string, string, UserIdentity,
            ...
            RoleClaimsDto<string>> GetIdentityService(
            IIdentityRepository<string, string, UserIdentity, UserIdentityRole, string, UserIdentityUserClaim,
            ...
            IMapper mapper, IAuditEventLogger auditEventLogger, ILoggerService loggerService)
        {
        ...
        private IMapper GetMapper()
        {
            return new MapperConfiguration(cfg => cfg
                    .AddProfile<IdentityMapperProfile<UserDto<string>, string, RoleDto<string>, string, UserIdentity,
                            UserIdentityRole, string,
                            UserIdentityUserClaim, UserIdentityUserRole, UserIdentityUserLogin, UserIdentityRoleClaim,
                            UserIdentityUserToken,
                            UsersDto<UserDto<string>, string>, RolesDto<RoleDto<string>, string>,
        ...
        {
            await using (var context = new AdminIdentityDbContext(_dbContextOptions))
            {
                var identityService = GetIdentityService(context);
        ...
        public async Task AddUserRoleAsync()
        {
            using (var context = new AdminIdentityDbContext(_dbContextOptions))
            {
                ...
                var userRoleDto =
                    IdentityDtoMock<string>.GenerateRandomUserRole<RoleDto<string>>(roleDto.Id, userDto.Id);
        ...
        public async Task DeleteUserRoleAsync()
        {
            using (var context = new AdminIdentityDbContext(_dbContextOptions))
            {
                ...
                //Get deleted role
                var userRoleDeleted = await context.UserRoles
                    .Where(x => x.RoleId == roleDto.Id && x.UserId == userDto.Id).SingleOrDefaultAsync();
